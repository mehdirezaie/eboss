...
class Netregression(object):
    ...
    def train_evaluate(self, learning_rate=0.001,
        ...
        baseline_testrmse  = np.sqrt(np.mean(test_w*(test_Y)**2))
        baseline_trainrmse  = np.sqrt(np.mean(train_w*(train_Y)**2))
...
data_eboss =np.load(path3+'test_train_eboss_4fold-wo-ndet-frac.npy').item()
...
for i in range(4): # 4 fold
    ...
    run_nchainlearning(path3+'wo-ndet-frac/'+fold+'/',
